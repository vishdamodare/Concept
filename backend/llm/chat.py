...
class LlmChat:
    ...
    async def send_message(self, message: UserMessage) -> str:
        model_name, api_key = self._get_api_setup()
        messages = [
            {"role": "system", "content": self.system_message},
            {"role": "user", "content": message.text}
        ]
        log.info(f"Sending message to {model_name} via LiteLLM")
        try:
    ...
    async def send_message_multimodal_response(self, message: UserMessage) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        log.info(f"Sending multimodal message to {model_name} via LiteLLM")
        try:
            response = await self._execute_with_retry(
                litellm.acompletion,
                model=model_name,
